...
class ExercisesList extends StatelessWidget {
  final List<Exercises> listOfExercises;

  ExercisesList(this.listOfExercises);
      ...
      child: Scaffold(
        ...
        body: ListView.builder(
          ...
          itemBuilder: (context, _index) {
            Exercises item = listOfExercises[_index];
            return Container(
                ...
                children: <Widget>[
                    ...
                    child: InkWell(
                      onTap: () async {
                        navigatorPages(context,
                            await exerciseItem(item.id_lang, item.id_exercise));
                      },
                      child: Row(
                        children: <Widget>[
                          Padding(
                  ...
                  Padding(
                    padding: EdgeInsetsDirectional.only(
                        top: 0.0, start: 8.0 * 2 + (customImageSize), end: 0.0),
                    child: Divider(
                      height: 1.0,
...
Future<List<ExerciseItem>> exerciseItem(
    String _IdLang, String _SelectedExerciseId) async {
  List<ExerciseItem> _listOfExercises = await DbHelper.getInstance()
      .getExerciseItem(_IdLang, _SelectedExerciseId);
  return _listOfExercises;
}

void navigatorPages(context, List<ExerciseItem> _selectedExercise) {
  Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) =>
              ExercisePage(_selectedExercise)));
}
